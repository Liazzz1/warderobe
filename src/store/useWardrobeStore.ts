...
interface WardrobeState {
  ...
  loadLayersToCanvas: (layers: LookLayer[]) => void;
}
...
export const useWardrobeStore = create<WardrobeState>((set, get) => ({
  ...
  updateLookInStore: (look) =>
    set({ looks: get().looks.map((l) => (l.id === look.id ? look : l)) }),
  ...
  loadLayersToCanvas: (layers) => set({ builderLayers: layers, selectedLayerItemId: null }),
}));
