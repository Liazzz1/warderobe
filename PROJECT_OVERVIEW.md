...
1. **Каталог вещей**: добавление фото с камеры/галереи, автоматическое удаление фона, распределение по категориям (`outerwear`, `top`, `bottom`, `shoes`, `accessory`, `dress`), цветам и брендам.
...
3. **Папки и организация**: древовидная структура папок для луков (по сезонам, событиям и т.д.).
4. **Синхронизация между устройствами**: ПК (Telegram Desktop / Telegram Web) и смартфон (iOS / Android TMA) видят одну и ту же базу благодаря привязке к `telegram_id`.
5. **Offline-first**: локальный кэш на IndexedDB позволяет открывать вещи и образы даже при нестабильной сети.
